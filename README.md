...
- **fix** 修复高活性 ALDO 长周期振荡：64 个酶 + 满堆反应物时"产物 0↔1 个、反应物 128↔127 个"来回跳——平衡点附近净速率≈0 不触发原有细分判据，大步长 RK4 数值误差缓慢把系统推离平衡形成长周期极限环；新增"平衡区驻留"判据（高 Vmax 背景 + 净通量≈0 时强制细步至稳定域），引擎 10000 tick 长跑 Q 精确锁定 Keq 零漂移（引擎测试 25 用例含长周期回归）
