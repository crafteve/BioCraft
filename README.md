...
- **fix** GUI 槽位原版化（快捷键兼容）：物种槽 isActive 打开并修复原版/模组快捷键——悬停 tooltip、数字键 1-9 快速放入快捷栏、JEI/EMI 的 U/R 查看配方（经 hoveredSlot 机制）恢复原版行为；1.21.1 的 Slot 坐标是 final 无法动态改，滚动槽位的命中/悬停由 GUI 手动计算并覆写 hoveredSlot；点击（拾取/放置/Shift 快速转移/右键拆分）维持原手动路径；双击收集与触屏拖拽分裂仍不支持（vanilla 私有状态机限制）
