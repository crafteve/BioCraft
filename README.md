...
- **fix** 酶反应腔方块外观现在随 0 槽酶实时变化：插入/更换/取走酶后，观察窗/管道/指示灯立即切换为对应酶主题色（或取空后的空机暗灰）——此前主题色只存在服务端方块实体中，客户端渲染线程始终读取旧缓存（恒显示空机状态），现通过方块实体更新包（update tag + data packet）随换酶同步到客户端并触发重渲染
