...
28. **RK4 刚性失效坑（已踩坑 + 已解：引擎自适应细分）**：RK4 在高刚性系统下数值失效——通量报告正常（甚至巨大）但浓度几乎不推进，症状是"v 显示很大但机器卡死"。根因：kcat 极大（TPI kcat=9000，Vmax_b = Vmax_f·∏KmP/(∏KmS·Keq) ≈ 74）时，RK4 中间采样点在巨大正逆向通量间震荡（G3P 中间值变负），四阶项大幅自抵消，终值增量 ≈ 0（实测 ΔG3P ≈ 1.6e-5），而 StepResult 用终值浓度算出的净通量仍显示 3168/tick——引擎"看似在跑实则卡死"。**引擎修复（EnzymeSimulator）**：刚性自适应细分——探测"高阶项自抵消"（RK4 增量 < 0.25×欧拉预测即视为步长过大），步长二分（上限 64 子步）直到子步稳定，逐子步 RK4 + 全 tick 最小边界缩放追踪；非刚性系统子步数=1 与原逻辑完全一致（PGI 黄金值回归守护）。**测试守护**：engineTest 用例 23（kcat=9000 数据必须正常推进并收敛平衡）。**数据侧约定**：TPI kcat 用大肠实测 9000 保持数据真实性，引擎已能承载；超扩散极限的极值数据仍需在数据准备阶段核对并标注
29. **1.21.1 Slot 坐标是 final（滚动槽位原版化的关键约束，已踩坑 + 已解）**：`Slot.x/y` 是 `public final int`（构造后不可改），且 `AbstractContainerScreen.findSlot(Slot 版 isHovering)` 是 private——vanilla 的"命中/hoveredSlot"永远按构造时的固定坐标工作，**无法通过改坐标让 vanilla 命中动态滚动槽位**。但 `hoveredSlot` 字段是 protected 可写、`renderSlot(Slot)` 是 protected 可覆写、`Slot.isHighlightable()` 可覆写。**可行组合（MachineScreen 已用）**：物种槽固定构造于屏外 (-100,-100)（vanilla findSlot 永不命中、渲染无残留）→ 点击经 mouseClicked 手动 findDynamicSlot + slotClicked（拾取/放置/Shift 转移/右键拆分）；悬停经 render 尾部手动命中并覆写 hoveredSlot（数字键快速放入 / JEI-EMI U-R / renderTooltip 悬停 tooltip 三项恢复原版行为）；物品/高亮仍自绘（renderSlot 对物种槽空实现防双重渲染，isHighlightable=false 关闭不可裁剪的 vanilla 高亮——renderSlotHighlight 是 static 且渲染循环内联，无法注入 scissor 裁剪）。**已知缺口**：双击收集与触屏拖拽分裂依赖 vanilla 私有状态机（quickcraft/lastClickSlot 等）与坐标命中，滚动槽位不可用。结论：1.21.1 想"原版化动态槽位"只能走"固定屏外坐标 + 手动命中 + 覆写 hoveredSlot"路线，覆写 render() 复制循环不可行（draggingItem/snapbackItem 等私有字段无法读取）
