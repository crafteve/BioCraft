...
28. **RK4 刚性失效坑（已踩坑 + 已解：引擎自适应细分 + 平衡区判据）**：RK4 在高刚性系统下数值失效——通量报告正常（甚至巨大）但浓度几乎不推进，症状是"v 显示很大但机器卡死"。根因：kcat 极大（TPI kcat=9000，Vmax_b = Vmax_f·∏KmP/(∏KmS·Keq) ≈ 74）时，RK4 中间采样点在巨大正逆向通量间震荡（G3P 中间值变负），四阶项大幅自抵消，终值增量 ≈ 0（实测 ΔG3P ≈ 1.6e-5），而 StepResult 用终值浓度算出的净通量仍显示 3168/tick——引擎"看似在跑实则卡死"。**引擎修复（EnzymeSimulator）三判据**：①自抵消判据（RK4 增量 < 0.25×欧拉预测 → 步长过大，覆盖振荡型刚性）；②单调大消耗判据（欧拉单步吃掉过半存量/余量 → 细分，覆盖"Vmax_b >> Vmax_f"型——ALDO 类 Keq 极小酶单步把产物吃光钳 0 的"产-吃光"极限环）；③平衡区驻留判据（高 Vmax 背景 >50/s 且净通量 < 背景×1e-4 → 直接取最大子步数 64——平衡点附近净速率≈0 时判据 1/2 都不触发，大步长 RK4 放大因子 |R(h·λ)| 在 h·λ≈1475 时远超 1，数值误差缓慢推离平衡形成**长周期极限环**（实测 ALDO 64 活性 Q 从 1.44e-4 缓慢漂移到 6.4e-5 再回弹、周期 >4000 tick，槽位投影表现为"产物 0↔1 个、反应物 128↔127 个"来回跳——旧 2000 tick 测试"恰好落在高位"是收敛假象）；64 子步 h=7.8e-4 → h·λ≈0.68 稳落稳定域）。非刚性系统子步数=1 与原逻辑完全一致（PGI 黄金值回归守护）。**测试守护**：engineTest 用例 23（TPI kcat=9000 推进收敛）+ 24（ALDO 64 活性收敛）+ 25（ALDO 长周期零漂移）。**数据侧约定**：TPI kcat 用大肠实测 9000 保持数据真实性，引擎已能承载；超扩散极限的极值数据仍需在数据准备阶段核对并标注
