...
- 已完成 酶设计单 DSL（字段型声明式语言，2026-08-19）：**program/ 纯核心**（零 MC 依赖，同 reaction/seq 门禁）——`ProgramField` 硬编码字段枚举（id/name/kcat/input/output + 值类型 + 解锁标记，非 JSON 查表）、`EnzymeProgramParser` 逐行 `keyword: value` 解析（重复字段后者覆盖、大小写不敏感、注释/空行、kcat 百分比 `[+-]\d{1,3}%`、input/output 物种各 ≤2）、`ProgramError` 错误码（MISSING_ID/ID_NOT_FOUND/UNKNOWN_FIELD/BAD_VALUE/TOO_MANY_SPECIES/UNKNOWN_SPECIES/CHEM_UNBALANCED）、`ChemBalanceChecker` 化学守恒纯逻辑（反应项 = 组成 × 系数，∑反应物 = ∑产物）；`tools/programTest` 32 用例全绿；**MC 侧装配**（`data/EnzymeProgramChecker`：id 解析到 enzymes.json 模板酶 + 物种存在性 substances.json + 完整反应式 = 模板反应式 + input/output 新增项（方案甲，CDK 原子守恒，`MoleculeDataCalculator.atomCounts`）；编辑器适配——模板 `id: HK`/`name: 己糖激酶`、字段关键词高亮（CodeEditorWidget 与 ProgramHighlight 共用分词）、kcat 行灰显"（未解锁）"、编码预览实时显示程序错误；**TNT 诱变/折叠机/语义注册表后置**（kcat 解锁 = 翻译成功成就，编辑器不拦截未解锁字段防死锁）
