...
26. **槽位容量参数化的隐藏钳制点（已踩坑）**：让槽位容纳 n 组物品（128 个），只改容器 `getMaxStackSize` **不够**——vanilla 所有物流路径都以"物品自身堆叠上限"参与 `min(物品, 槽位)` 运算：`Container.getMaxStackSize(ItemStack)` 默认 min、`Slot.safeInsert`/`getMaxStackSize(ItemStack)`、`moveItemStackTo`（shift）、`HopperBlockEntity.tryMoveInItem` 合并、`ItemHandler.insertItem` 的 limit。而 1.21 的物品堆叠上限是**数据组件 `MAX_STACK_SIZE`**（构造时 `Properties.component(...)` 或 `stacksTo(n)` 写入），不是覆写方法。修复组合拳：①`MoleculeItem` 构造写 `MAX_STACK_SIZE = 64×SLOT_GROUPS`（分子物品全局 128，语义"堆叠数=分子个数"成立）；②容器覆写 `getMaxStackSize(ItemStack)` 返回 slotStackLimit（绕过 min 物品）；③GUI `RestrictedSlot` 覆写 `getMaxStackSize(ItemStack)` 返回槽位容量；④`EnzymeFactoryItemHandler.getSlotLimit` 返回 64×n。DNA 编码器不覆写 slotStackLimit 保持 64，碱基 128 进其槽被 limitSize 截回 64，行为不变
