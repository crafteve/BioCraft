...
- 已完成 DNA 解旋酶（helicase，2026-08-20）：无催化剂方块形态（`SequenceMachineKind.HELICASE` 容器 3 槽：1 dsDNA 输入 + 2 ssDNA 输出）、`HelicaseOperation` 原子 TRANSFORM（`SeqOps.reverseComplement` 产出 S + 反向互补链，kind 继承，双产物 NBT 不同需两产物卡）、`HelicaseScreen` 双产物卡（ssDNA-A/B 四色碱基）+ 中央解旋动画（待机双螺旋/解旋中分叉闪光/完成平行）、`SequenceMachineBlockEntity` 双产物 DONE 判定（两输出皆空才回 IDLE）、`ModBlocks`/`BioCraftClient`/`MachineModelProvider`/`ModCreativeTabs`/`SubstanceLanguageProvider` 全链路注册（占位 cube 模型）
...
│   ├── HelicaseOperation.java     # 解旋操作：1 dsDNA → 2 ssDNA 原子 TRANSFORM（S + reverseComplement，kind 继承，双产物）
...
│   ├── HelicaseScreen.java       # 解旋酶屏幕：双产物卡（ssDNA-A/B 四色碱基）+ 中央解旋动画（待机双螺旋/解旋中分叉闪光/完成平行）
