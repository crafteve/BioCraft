...
28. **RK4 刚性失效坑（已踩坑 + 已解数据层面）**：RK4 在高刚性系统下数值失效——通量报告正常（甚至巨大）但浓度几乎不推进，症状是"v 显示很大但机器卡死"。根因：kcat 极大（TPI 曾填 9000，Vmax_b = Vmax_f·∏KmP/(∏KmS·Keq) ≈ 74）时，RK4 中间采样点在巨大正逆向通量间震荡（G3P 中间值变负），四阶项大幅自抵消，终值增量 ≈ 0（实测 ΔG3P ≈ 1.6e-5），而 StepResult 用终值浓度算出的净通量仍显示 3168/tick——引擎"看似在跑实则卡死"。**诊断方法**：引擎测试用游戏真实数据复现（TestTpiRealDataDebug 模式：flux 大 + Δ浓度极小即中招）；**已解**：TPI kcat 9000 → 2946（酵母几何中位数，与 TestEnzymes 一致，RK4 稳定已验证）。**未解（引擎层）**：RK4 仍无刚性防护，新增酶 kcat 过大（>~3000 量级或 Vmax_b 过大）可能复发——未来应做自适应步长或刚性检测（通量×dt 超稳定性阈值时细分步长）；数据准备阶段应核对 kcat 量级（超扩散极限的极值需标注）
