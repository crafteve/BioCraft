...
- 待开发 中心法则第二波（信息链贯通，见《docs/中心法则信息层设计_2026-08-18.md》）：解旋酶 + 复制酶（1 dsDNA → 2 ssDNA → 各复制 → 2 dsDNA 倍增闭环，dNTP 消耗）→ tRNA 体系（trna_gene/trna/aa-tRNA/ARS 酶物品）→ 装载机（ARS 0 槽催化，aa + trna + ATP → aa-tRNA + AMP + PPi）+ 翻译仪（aa-tRNA 托盘 + 密码子↔反密码子教学 GUI）+ 折叠机（解码程序 → 语义注册表 → 酶物品或 misfolded_protein）——程序 → DNA → mRNA → 多肽 → 酶物品全链贯通
