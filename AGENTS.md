...
- 已完成 DNA 解旋酶（helicase，2026-08-20）：无催化剂方块形态（`SequenceMachineKind.HELICASE` 容器 3 槽：1 dsDNA 输入 + 2 ssDNA 输出）、`HelicaseOperation` 逐碱基对动态解旋（每 tick 1 bp，`SeqOps.reverseComplement` 产出 S 前缀 + 反向互补前缀，kind 继承，双产物 NBT 不同需两产物卡，链源 position/total 进度+输入完成时消耗）、`HelicaseScreen` guiv1 酶工厂布局（左侧 dsDNA 输入卡 + 右侧 ssDNA-A/B 垂直双卡 + 中央 122×126 大动画区逐碱基对显示 A–T/C–G + 分叉闪光）、`SequenceMachineBlockEntity` 双产物 DONE 判定（两输出皆空才回 IDLE）、`ModBlocks`/`BioCraftClient`/`MachineModelProvider`/`ModCreativeTabs`/`SubstanceLanguageProvider` 全链路注册（占位 cube 模型）
...
│   ├── HelicaseOperation.java     # 解旋操作：1 dsDNA → 2 ssDNA 逐碱基对动态解旋（每 tick 1 bp，S 前缀 + 反向互补前缀，kind 继承，双产物）
...
│   ├── HelicaseScreen.java       # 解旋酶屏幕：guiv1 布局（左 dsDNA + 右 ssDNA-A/B 垂直双卡 + 中央 122×126 大动画区逐碱基对 A–T/C–G + 分叉闪光）
