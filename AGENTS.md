...
- Phase 1 中心法则基础设施：DNA 组装器 / 转录仪 / 翻译仪（密码子表 + 20 种 tRNA）/ 折叠仪，独立方块/BE/GUI（不复用 EnzymeFactoryBlockEntity，因为需要序列可视化、密码子对照、进度追踪等完全不同的 GUI 信息），催化剂仍为酶蛋白物品（DNA 聚合酶/RNA 聚合酶/核糖体/分子伴侣），序列数据存储在物品 DataComponent，化学计量层由引擎处理、序列逻辑层在 BE 层实现；世界探索（远古细胞遗迹）获取 DNA 碎片 + 种子催化剂解决鸡生蛋问题
